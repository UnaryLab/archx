...
def query_performance_metrics(event_graph, metric_dict, workload, event, module, tag, metrics) -> OrderedDict:
    ...
    if any(metric in metrics for metric in ['cycle_breakdown', 'compute_cycle_count', 'memory_stall_cycle_count', 'sram_cycle_count']):
        ...
        if 'memory_stall_cycle_count' in metrics:
            performance_metrics_dict['memory_stall_cycle_count'] = cycle_breakdown['memory_stall_cycle_count']
...
def query_cycle_breakdown(event_graph, metric_dict, workload, event) -> OrderedDict:
    ...
    return OrderedDict({
        ...
        'memory_stall_cycle_count': memory_stall_cycle_count,
        'unattributed_cycle_count': total_cycle_count - compute_cycle_count - sram_cycle_count - memory_stall_cycle_count,
    })
...
def _component_event_counts(event_graph, event):
    ...
    while stack:
        name, path_count = stack.pop()
        if (name, path_count) in seen:
            continue
        seen.add((name, path_count))

        try:
            children = event_graph.get_out_neighbors(name)
        except ValueError:
            children = []

        for child in children:
            try:
                child_count = path_count * event_graph.get_edge_count(name, child)
            except ValueError:
                child_count = path_count
