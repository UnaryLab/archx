...
if not os.path.exists('agraph_casestudy/systolic_course_grain/results/'):
    os.makedirs('agraph_casestudy/systolic_course_grain/results/')
...
csv_path = 'agraph_casestudy/systolic_course_grain/results/'
...
for config in configs:
    path = f'agraph_casestudy/systolic_course_grain/description/runs/{config}'
    ...
    count = aggregate_event_count(event_graph=event_graph, workload=workload, event='pe')
    ...
    if memory_df is None:
        memory_df = pd.DataFrame(columns=['arch', 'memory_area', 'memory_dynamic_energy', 'memory_leakage_power'])
