...
def offchip_gemm_scheduling(batch, m, k, n, architecture_dict: OrderedDict, workload_dict: OrderedDict) -> TiledGEMM:
    ...
    m_array = architecture_dict['ififo']['query']['depth']
    k_array = architecture_dict['wfifo']['query']['depth']
    n_array = architecture_dict['ofifo']['query']['depth']
    ...
    if math.floor(tile_m * tile_k) > isram_elements:
        raise ValueError('Tile size exceeds isram size')
    if math.floor(tile_k * tile_n) > wsram_elements:
        raise ValueError(f'Tile size {tile_k * tile_n} exceeds wsram size {wsram_elements}')
    if math.floor(tile_m * tile_n) > osram_elements:
...
def onchip_gemm_events(onchip_tiling: list[TiledGEMM], architecture_dict: OrderedDict, workload_dict: OrderedDict) -> OrderedDict:
    ...
    performance_dict['subevent'] = OrderedDict({
        'input_reads': isram_onchip_reads_dict,
        'weight_reads': wsram_onchip_reads_dict,
        'output_reads': osram_onchip_reads_dict,
        'output_writes': osram_onchip_writes_dict
    })
