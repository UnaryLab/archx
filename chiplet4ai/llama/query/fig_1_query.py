...
for config in configs:
    ...
    with open(runs_path, 'r') as f:
        ...
        if config == '8b':
            array_query_8b_df.to_csv(output_path + f'array_performance_metrics_{config}.csv', index=False)
            ...
            if not array_query_8b_df.empty:
                ...
                array_query_8b_df_sci.to_csv(output_path + f'array_performance_metrics_{config}_scientific.csv', index=False)
            else:
                print("Warning: No matching configurations found. Scientific notation CSV not saved.")
        else:
            array_query_70b_df.to_csv(output_path + f'array_performance_metrics_{config}.csv', index=False)
            ...
            if not array_query_70b_df.empty:
                ...
                array_query_70b_df_sci.to_csv(output_path + f'array_performance_metrics_{config}_scientific.csv', index=False)
            else:
                print("Warning: No matching configurations found. Scientific notation CSV not saved.")