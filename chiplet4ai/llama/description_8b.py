...
def description(path):
    ...
    ififo_shapes = [[32], [64], [128], [256], [512]]
    wfifo_shapes = ififo_shapes
    #chiplet_shape = [16, 32, 64]
    chiplet_shape = [16]
    act_sram_width = bitwidth
    weight_sram_width = act_sram_width
    act_sram_bank = [32, 64, 128, 256, 512]
    weight_sram_bank = act_sram_bank
    mb = 1024 * 1024 * 8
    ...
    for m in memory_size:
        for b_act, b_weight in zip(act_sram_bank, weight_sram_bank):
            act_sram_depth.append(m // (b_act * act_sram_width))
            weight_sram_depth.append(m // (b_weight * weight_sram_width))
    ...
    # # FIFO
    fifos = architecture.add_module(name=['ififo', 'ofifo'], instance=ififo_shapes, tag=['onchip', 'fifo', 'array'], query={'class': ['fifo_sync_tsmc7', 'fifo_sync_tsmc16'], 'width': bitwidth, 'depth': chiplet_shape})
    wfifo = architecture.add_module(name='wfifo', instance=wfifo_shapes, tag=['onchip', 'fifo', 'array'], query={'class': ['fifo_sync_tsmc7', 'fifo_sync_tsmc16'], 'width': bitwidth, 'depth': chiplet_shape})

    # Output Accumulator
    output_adder = architecture.add_module(name='output_adder', instance=ififo_shapes, tag=['onchip', 'output_adder', 'array'], query={'class': ['fp16_adder_tsmc7', 'fp16_adder_tsmc16']})
    ...
    llama_3_8b_batch_size = llama_3_8b_config.add_parameter(parameter_name='batch_size', parameter_value=batch_size, sweep=False)
    ...
    agraph.direct_constraint(parameters=[fifos['ififo']['instance'],
                                         fifos['ofifo']['instance'],
                                         act_sram['isram']['query']['bank'],
                                         output_adder['instance']
                                         ])
    
    agraph.direct_constraint(parameters=[wfifo['instance'],
                                         wsram['query']['bank']
                                        ])
    ...
    agraph.conditional_constraint(a = wfifo['instance'],
                                  b = muxes['weight_mux']['instance'],
                                  condition = lambda a, b: a[0] == b[1])
    
    agraph.conditional_constraint(a = fifos['ififo']['instance'],
                                  b = fifos['ififo']['query']['depth'],
    ...
    agraph.conditional_constraint_equation(a = act_sram['isram']['query']['bank'],
                                           b = act_sram['isram']['query']['depth'],
                                           c = memory_size_wkld['memory_size_mb'],
                                           condition = lambda a, b, c: a * b * act_sram_width == c)
    
    agraph.conditional_constraint_equation(a = wsram['query']['bank'],
                                           b = wsram['query']['depth'],
                                           c = memory_size_wkld['memory_size_mb'],
                                           condition = lambda a, b, c: a * b * weight_sram_width == c)
